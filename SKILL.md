...
**Checkpoint mode.** When a hook or the user asks for a checkpoint mid-session (the session has grown since its last save, and RESUME.md should be made current while the cache is still warm), run the sweep below exactly as usual, with the scope always "since the last save point" (the previous RESUME.md write), never the whole chat, then end that reply. A checkpoint is a save point, not the end of the session: the session continues as normal with the user's next message, so the report should not suggest `/clear` or a new session. Starting fresh only pays off when the user comes back after the prompt cache has expired (see above).
